...
public class TopicService implements ManagedService, RemoteService, EventPublishingService {
    ...
    public void publishEvent(String name, Object messageObject, TopicEvent event) {
        Collection<EventRegistration> registrations = eventService.getRegistrations(TopicService.SERVICE_NAME, name);
        Collection<EventRegistration> filteredRegistrations = new ArrayList<EventRegistration>();
        for(EventRegistration r : registrations) {
            // filter out registrations whose filter doesn't eval true
            boolean acceptRegistration = true;
            try {
                acceptRegistration = r.getFilter().eval(messageObject);
            } catch(Exception e) {
                // this exception is probably generated by the filter. ignore and eval to true
            }
            if(acceptRegistration) filteredRegistrations.add(r);
        }
        eventService.publishEvent(TopicService.SERVICE_NAME, filteredRegistrations, event, name.hashCode());
    }
    ...
    public String addMessageListener(String name, MessageListener listener, MessageEventFilter filter) {
        EventRegistration eventRegistration = eventService.registerListener(TopicService.SERVICE_NAME, name, new MessageEventFilterDelegate(filter), listener);
        return eventRegistration.getId();
    }
    ...
    private static class MessageEventFilterDelegate implements EventFilter {

        private final MessageEventFilter delegate;

        private MessageEventFilterDelegate(MessageEventFilter filter) {
        ...
        public boolean eval(Object arg) {
            ...
            return delegate.shouldPublish(arg);
        }
    }
}
